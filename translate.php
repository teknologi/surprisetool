...
+    $qry = sprintf("SELECT DISTINCT(first.id) as id, first.text as first_text, second.text as second_text, first.comment as first_comment, second.comment as second_comment FROM lingual as first LEFT JOIN lingual as second ON first.id = second.id AND second.iso = '%s' WHERE first.iso = '%s' AND first.active='1'%s limit %d, %d;", $safe["lato"], $safe["lafrom"], $qry_find, ($safe["npage"]-1)*100, ($safe["npage"])*100);
    $res = $db->query($qry);
    if($res) {
  ...
  //        echo '<select name="langfrom" ONCHANGE="location = this.options[this.selectedIndex].value;">';
        ...
        if($result && $result->num_rows) {
            while($reslang = $result->fetch_object()) {
                echo '<a href="'.SITEISO.'translate/'.$reslang->iso.'/'.$safe['lato'].'/1/"'.(($safe["lafrom"]==$reslang->iso)?' class="selected"':'').' style="width:50px;" title="'.$reslang->language.'">'.$reslang->iso.'</a>';
            }
        }
        echo '</select>';
        echo '</th>';
  ...
  //        echo '<select name="langto" ONCHANGE="location = this.options[this.selectedIndex].value;">';
        ...
        if($result && $result->num_rows) {
            while($reslang = $result->fetch_object()) {
                echo '<a href="'.SITEISO.'translate/'.$safe['lafrom'].'/'.$reslang->iso.'/1/"'.(($safe["lato"]==$reslang->iso)?' class="selected"':'').' style="width:50px;" title="'.$reslang->language.'">'.$reslang->iso.'</a>';
            }
        }
  //        echo '</select>';
        echo '</th></tr>';
